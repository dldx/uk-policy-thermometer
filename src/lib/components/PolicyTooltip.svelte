...
<div class="relative max-w-lg tooltip-content">
    <!-- Close button -->
    <button
    ...
    {#if datum.scores?.[criterion]}
        <div
            class="bg-linear-to-br from-blue-50 to-indigo-50 mb-4 p-4 border border-blue-200 rounded-lg"
        >
            <div class="flex justify-between items-center mb-3">
                <div class="flex items-center gap-2">
                    <svg
                        class="shrink-0 w-5 h-5 text-blue-600"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    >
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <div class="flex gap-1">
                            {#each Array(Math.max(3, Math.ceil(datum.scores[criterion].weight))) as _, i}
                                <div
                    ...
                    <div
                        ...
                        style:background-color={datum.party_color}
                    >
                        {datum.scores[criterion].score.toFixed(1)}<span
                            class="opacity-90 text-sm">/10</span
                        >
                    </div>
                </div>
            </div>
    ...
    {#if datum.source && (datum.source.url || datum.source.notes)}
        <details class="group">
            <summary
                class="flex items-center gap-2 bg-gray-100 hover:bg-gray-150 px-3 py-2 border border-gray-200 rounded-lg font-semibold text-gray-700 text-xs transition-colors cursor-pointer"
            >
                <svg
            ...
            <div
                class="bg-gray-50 mt-2 p-3 border border-gray-200 rounded-lg text-xs"
            >
                {#if datum.source.url}
                    <a
